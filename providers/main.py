...
parser = argparse.ArgumentParser(description='Create snapshots from kubernetes PVCs.')
parser.add_argument('--dont-create-snapshots', dest='create_snapshots', action='store_false', default=True,
                    help='Don\'t create snapshots.')
...
if args.retention_days < 1:
    ...
    print(f'Invalid retention days. Setting to default: {args.retention_days}')

if os.environ.get('AWS_ACCESS_KEY_ID') and os.environ.get('AWS_SECRET_ACCESS_KEY'):
    print('--> Detected provider AWS')
    from .aws import AWS as Provider
    provider = Provider()
else:
    print('--> Unable to detect provider')
    sys.exit(1)
